...
public class GeminiFinancialAdviceClient {
    ...
    public FinancialAdviceAiContent generateAdvice(String financialContextJson) {
        if (!StringUtils.hasText(apiKey)) {
            throw new IllegalStateException("Configura la variable de entorno GEMINI_API_KEY para activar el asesor financiero con IA.");
        }

        try {
            String payload = objectMapper.writeValueAsString(buildRequestBody(financialContextJson));
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                throw new IllegalStateException("Gemini respondió con estado " + response.statusCode() + ": " + response.body());
            }

            String rawJson = extractText(response.body());

            if (!StringUtils.hasText(rawJson)) {
                return FinancialAdviceAiContent.fallback("No fue posible obtener una respuesta útil de Gemini en este momento.");
            }

            return objectMapper.readValue(cleanJson(rawJson), FinancialAdviceAiContent.class);
        } catch (IOException ex) {
            throw new IllegalStateException("No fue posible interpretar la respuesta de Gemini.", ex);
        } catch (InterruptedException ex) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("La solicitud a Gemini fue interrumpida.", ex);
        }
    }
    ...
    private Map<String, Object> responseSchema() {
        ...
        insight.put("required", List.of("title", "description", "severity", "icon"));
        ...
        actionItem.put("required", List.of("title", "description", "priority", "estimatedImpact"));
    ...
    private String extractText(String responseBody) throws IOException {
        ...
        if (!finishReason.isMissingNode() && finishReason.isTextual()) {
            throw new IllegalStateException("Gemini no devolvió texto. Motivo: " + finishReason.asText());
        }
        ...
        if (!blockReason.isMissingNode() && blockReason.isTextual()) {
            throw new IllegalStateException("Gemini bloqueó la respuesta: " + blockReason.asText());
        }
